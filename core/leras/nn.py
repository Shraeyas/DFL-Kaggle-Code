...
class nn():
    ...
    def ask_choose_device_idxs(choose_only_one=False, allow_cpu=True, suggest_best_multi_gpu=False, suggest_all_gpu=False):
        ...
        io.log_info ("")
        '''
        io.log_info ("Choosing Best GPU for You.")
        ...
        while True:
            try:
                if choose_only_one:
                    choosed_idxs = '0'#io.input_str("Which GPU index to choose?", best_device_indexes)
                else:
                    choosed_idxs = '0'#io.input_str("Which GPU indexes to choose?", best_device_indexes)

                #print ("Chosen index : " + str(choosed_idxs))
