...
class ModelBase(object):
                ...
                while True:
                    ...
                    if len(saved_models_names) != 0:
                        if silent_start:
                            ...
                            inp = '0'#io.input_str(f"", "0", show_default_value=False )
                        ...
                        self.model_name = 'v1'#io.input_str(f"No saved models found. Enter a name of a new model", "new")
    ...
    def ask_autobackup_hour(self, default_value=0):
        ...
        self.options['autobackup_hour'] = 0#io.input_int(f"Autobackup every N hour", default_autobackup_hour, add_info="0..24", help_message="Autobackup model files with preview every N hour. Latest backup located in model/<>_autobackups/01")
    ...
    def ask_write_preview_history(self, default_value=False):
        ...
        self.options['write_preview_history'] = 0#io.input_bool(f"Write preview history", default_write_preview_history, help_message="Preview history will be writed to <ModelName>_history folder.")
    ...
    def ask_target_iter(self, default_value=0):
        ...
        self.options['target_iter'] = 0#max(0, io.input_int("Target iteration", default_target_iter))

    def ask_random_flip(self):
        default_random_flip = self.load_or_def_option('random_flip', True)
        self.options['random_flip'] = 'y'#io.input_bool("Flip faces randomly", default_random_flip, help_message="Predicted face will look more naturally without this option, but src faceset should cover all face directions as dst faceset.")
    ...
    def ask_batch_size(self, suggest_batch_size=None, range=None):
        ...
        batch_size = 4#max(0, io.input_int("Batch_size", default_batch_size, valid_range=range, help_message="Larger batch size is better for NN's generalization, but it can cause Out of Memory error. Tune this value for your videocard manually."))
